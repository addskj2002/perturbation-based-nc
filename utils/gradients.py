...
def yield_gradients(X, model, loss_fn, device, save=True, overwrite=True, cache=None):
    ...
    if (not overwrite) and (
        ...
        if "gradient" in filedata:
            ...
            idx = 0
            while os.path.exists(f"{filedata["gradient"]}/{idx}.pth"):
                new_gradients = torch.load(
                    f"{filedata["gradient"]}/{idx}.pth",
                    weights_only=False,
                    map_location=device,
                )
    ...
    gradients = []
    kdx = 0
    for idx, single_N in tqdm(enumerate(Ns)):
        for jdx in range(single_N):
