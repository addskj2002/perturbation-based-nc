...
do
    ...
    do
        ...
        do
            ...
            do
                echo Transfer $ssl $pretrain $arch $downstream $stddev $seed $model_num in_space
                python3 main_nc_uncertainty.py \
                    ...
                    --metric cosine \
                    --seed $seed \
                    --outfile nc_uncertainty/$ssl\_$pretrain\_$arch\_$downstream/test/ckpt_$model_num/in_space_$stddev\_nc_$seed.pth
                    
                echo Transfer $ssl $pretrain $arch $downstream $stddev $seed $model_num ortho_space
                python3 main_nc_uncertainty.py \
                    --original-model models/$ssl\_$pretrain\_$arch/$model_num.pth \
                    ...
                    --metric cosine \
                    --seed $seed \
                    --outfile nc_uncertainty/$ssl\_$pretrain\_$arch\_$downstream/test/ckpt_$model_num/ortho_space_$stddev\_nc_$seed.pth
                    
                echo Transfer $ssl $pretrain $arch $downstream $stddev $seed $model_num gradinv
                python3 main_nc_uncertainty.py \
                    --original-model models/$ssl\_$pretrain\_$arch/$model_num.pth \
                    ...
                    --metric cosine \
                    --seed $seed \
                    --outfile nc_uncertainty/$ssl\_$pretrain\_$arch\_$downstream/test/ckpt_$model_num/gradinv_$stddev\_nc_$seed.pth
            done
        done
    done
done
