...
from uncertainty.vonMF import spherical_kmeans
...
def _log_likelihood(X, centers, weights, concentrations, normalize=True):
    if len(np.shape(X)) != 2:
        X = X.reshape((1, len(X)))
...
def _maximization(X, posterior, force_weights=None):
    ...
    weights = weights + 1e-10  # avoid divide by zero
...
class VonMisesFisherMixture(KMeans):
    ...
    def predict_log_proba(self, X):
        ...
        logp = _log_likelihood(
            X, self.cluster_centers_, self.weights_, self.concentrations_,  normalize=False
        )
        return logsumexp(logp, axis=0)
    ...
    def predict_proba(self, X):
        ...
        return np.sum(np.exp(logp), axis=0)
